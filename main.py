...
class DBApp(ctk.CTk):
    ...
    def _display_entity_data(self, entity_name):
        ...
        if entity_name == "Финансы":
            ...
            schedule_data = views.get_schedule_data(self.conn, self.calendar_date, self._get_employee_map)
            self.scrollable_cards_frame.configure(label_text=f"График работы на месяц")
            views.display_calendar_view(self, entity_name, schedule_data)
            return

        elif entity_name == "Расписание":
            records = views.get_appointment_data(self.conn, self.schedule_date, self._get_employee_map)
