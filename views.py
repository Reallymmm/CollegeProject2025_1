...
def get_schedule_data(conn, calendar_date, get_employee_map_func):
    ...
    cur = conn.cursor()
    cur.execute(
        'SELECT * FROM "График работы" WHERE Дата BETWEEN ? AND ?',
        (str(start), str(end)),
    )
...
def display_calendar_view(app, entity_name, schedule_data):
    ...
    ctk.CTkButton(
        app.top_controls,
        text="<",
        command=lambda: app.change_calendar_month(-1),
    ).grid(row=0, column=0)

    ctk.CTkLabel(
        app.top_controls,
        text=app.calendar_date.strftime("%B %Y"),
        font=("Arial", 18, "bold"),
    ).grid(row=0, column=1)
    ...
    ctk.CTkButton(
        app.top_controls,
        text="➕ Смена",
        command=lambda: open_add_schedule_dialog(app),
    ).grid(row=0, column=2)
...
def get_appointment_data(conn, date, get_employee_map_func):
    ...
    cur.execute(
        ...
        LEFT JOIN "Сотрудники" e ON z.ID_Сотрудника = e.ID
        WHERE z.Дата = ?
        ORDER BY z.Время
    """,
        (str(date),),
    ...
    for r in base_records:
        ...
        details = (
            ...
            f"Сотр: {r['Сотрудник'] or 'Неизвестно'}, "
        )
        if service_names:
...
def display_entity_cards(app, entity_name, records, columns):
    ...
    if entity_name == "Сотрудники":
        column_names = [name for name in column_names if name != "Должность"]
    ...
    for index, record in enumerate(records):
        ...
        for name in column_names:
            ...
            if entity_name == "Записи":
                ...
                if name == "ID_Сотрудника":
                    ...
                    cursor.execute(
                        'SELECT "Имя" FROM "Сотрудники" WHERE ID = ?',
                        (record[name],),
                    )
                    emp_row = cursor.fetchone()
                    value = emp_row["Имя"] if emp_row else "Неизвестно"
        ...
        if entity_name == "Услуги":
            ...
            if materials:
                materials_text = ", ".join(
                    f"{m['Название_Товара']} ({m['Количество']} {m['Единица_измерения']})"
